...
namespace RoadSpeedAdjuster.Extensions
{
    ...
    public abstract partial class ExtendedUISystemBase : UISystemBase
    {
        ...
        {
            var helper = new ValueBindingHelper<T>(new(Mod.Id, $"BINDING:{key}", initialValue, new GenericUIWriter<T>()));
        ...
        {
            var helper = new ValueBindingHelper<T>(new(Mod.Id, $"BINDING:{key}", initialValue, new GenericUIWriter<T>()), updateCallBack);
            var trigger = new TriggerBinding<T>(Mod.Id, $"TRIGGER:{setterKey}", helper.UpdateCallback, new GenericUIReader<T>());
        ...
        {
            var binding = new TriggerBinding(Mod.Id, $"TRIGGER:{key}", action);
        ...
        {
            var binding = new TriggerBinding<T1>(Mod.Id, $"TRIGGER:{key}", action, new GenericUIReader<T1>());
        ...
        {
            var binding = new TriggerBinding<T1, T2>(Mod.Id, $"TRIGGER:{key}", action, new GenericUIReader<T1>(), new GenericUIReader<T2>());
        ...
        {
            var binding = new TriggerBinding<T1, T2, T3>(Mod.Id, $"TRIGGER:{key}", action, new GenericUIReader<T1>(), new GenericUIReader<T2>(), new GenericUIReader<T3>());
        ...
        {
            var binding = new TriggerBinding<T1, T2, T3, T4>(Mod.Id, $"TRIGGER:{key}", action, new GenericUIReader<T1>(), new GenericUIReader<T2>(), new GenericUIReader<T3>(), new GenericUIReader<T4>());
